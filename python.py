...
def encode_score2(value):

    if value > 29900 or value < -29900:
        if value > 29900:
            return 30000 - value
        else:
            return -30000 - value
...
def decode_score(score, isBlack = False):
    if score > 29900:
        return {"mate": 30000 - score}
    elif score < -29900:
        return {"mate": -30000 - score}
        ...
        if command == "bestmove":
            ...
            while tokens:
                ...
                if parameter == "multipv":
                    ...
                    # kind = tokens.pop(0)
                    # score = encode_score2(int(tokens.pop(0)))
                    score = int(tokens.pop(0))
        ...
        if command == "bestmove":
            ...
            while tokens:
                ...
                if parameter == "multipv":
                    ...
                    depth = int(tokens.pop(0))
                    #if depth == 5
                        #result5 = make_low_depth_request(depth, score, pvs)
                        #fire_and_forget(url = "http://192.168.1.5:9663/fishnet/move", json=result5)
...
class Worker(threading.Thread):
    ...
    def report_and_fetch(self, path, result, params):
        if "fen" in result:
            logging.warning("sending request to 9665")
            #requests.post("http://192.168.1.5:9666/put", data=result)
            fire_and_forget(url = "http://movecache.kyvuong.net/put", json=result)
    ...
    def work(self):
        ...
        if self.job and self.job["work"]["type"] == "analysis" and self.job["fen"] is not None:
            result = self.analysisMove(self.job)
         ...
         #   result["analysis"] = {
          #      "fen" : job["fen"],
           #     "score": [[] for _ in range(len(moves) + 1)],
            #    "pv": [[] for _ in range(len(moves) + 1)],
            #}
        go(self.stockfish, job["fen"], "", depth=depth)
       
        scores, pvs = recv_analysisMove(self.stockfish)
       
        #result["analysis"]["score"] = [item.pop() for item in scores]
        #result["analysis"]["pv"] = [" ".join(item.pop().split()[0:5]) for item in pvs]
        score = [item.pop() for item in scores]
        pvs = [" ".join(item.pop().split()[0:6]) for item in pvs]
        moveResult = []
        fenSplit = job["fen"].split(" ")
        isBlack = False
        if fenSplit[1] == "b":
            #logging.warning("IS BLACKKKKKKKKKKKKKKKKKKKKKKK")
            isBlack = True
        for idx, val in enumerate(score):
            amove = {}
            #amove["depth"]= depth
            amove["score"] = decode_score(val, isBlack)
            amove["nodes"] = 0
            amove["nps"] = 0
            amove["pv"] = pvs[idx]
            moveResult.append(amove)   
        result["analysis"] = moveResult
        result["fen"] = job["fen"]
