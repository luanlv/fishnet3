...
def recv_analysis(p):
    logging.warning("Run recv_analysis")
    scores = []
    nodes = []
    times = []
    pvs = []
    ...
    while True:
        command, arg = recv_uci(p)
        logging.warning("CURRENT SCORES")
        ...
        if command == "bestmove":
            ...
            stringArg = " ".join((arg or "").split())
            tokens = stringArg.split(" ")
            ...
            while tokens:
                ...
                if parameter == "multipv":
                    ...
                    logging.warning("SET TABLE SCORE = ")
                    kind = tokens.pop(0)
                    score = 0
                    if kind == "mate":
...
class Worker(threading.Thread):
    ...
    def analysis(self, job):
        ...
        for ply in range(len(moves), -1, -1):
            ...
            if multipv is None:
                logging.warning("SCORESSSSSSSSSSSSSS")
                logging.warning(scores)
